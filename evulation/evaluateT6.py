...
def extraction():
	f1 = open('F:\Comparative-factor-extraction-car\CRF++\outputT6.txt', 'r')
	f2 = open('F:\Comparative-factor-extraction-car\CRF++\\test_answer.txt', 'r')
	f3 = open('F:\Comparative-factor-extraction-car\CRF++\evaluateT6.txt', 'w')
	...
	for line1 in f1.readlines():
		if line1 == '\n':
			...
			result = Defi_class.Evaluation('', result_1[0],result_1[1],result_1[4], result_1[2],   result_1[3], result_1[5],'')
			# print(result.word)
			list.append(result)
			i += 1
	# print(len(list))
	...
	for line2 in f2.readlines():
		...
		if line2 == '\n':
			...
			f3.write(
				list[num].index + ' ' + list[num].word + ' ' + list[num].pos+ ' '+ list[num].keyword+  ' '+list[num].position+ ' ' + list[num].chunk+ ' ' + list[num].result1 + ' ' + list[
					num].result2)
			f3.write('\n')
	return list

f4=open('F:\Comparative-factor-extraction-car\CRF++\\result2T6.txt', 'w')
f5=open('F:\Comparative-factor-extraction-car\CRF++\\result1T6.txt', 'w')
list=extraction()
list2=[]
...
for j3 in range(len(list_pre)):
	if 'NULL'in list_pre[j3].word:
		continue
	if '是'==list_pre[j3].word or '一'==list_pre[j3].word or ','==list_pre[j3].word or '能'==list_pre[j3].word or '的'==list_pre[j3].word:
		continue
	else:
