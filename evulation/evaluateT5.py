...
def extraction():
	f1 = open('F:\Comparative-factor-extraction-car\CRF++\outputT5.txt', 'r')
	f2 = open('F:\Comparative-factor-extraction-car\CRF++\\test_answer.txt', 'r')
	f3 = open('F:\Comparative-factor-extraction-car\CRF++\evaluateT5.txt', 'w')
	'''
	# 测试结果和答案是一样长的文件
	num = 0
	list2 = []
	...
	for line2 in f2.readlines():
		...
		if line2 == '\n':
			...
			f3.write(
				list[num].index + ' ' + list[num].word + ' ' + list[num].pos+ ' ' + list[num].chunk+ ' ' + list[num].result1 + ' ' + list[
					num].result2)
			f3.write('\n')
	return list
f4=open('F:\Comparative-factor-extraction-car\CRF++\\result2T5.txt', 'w')
f5=open('F:\Comparative-factor-extraction-car\CRF++\\result1T5.txt', 'w')
