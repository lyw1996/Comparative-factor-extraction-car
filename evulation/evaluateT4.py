...
def extraction():
	f1 = open('F:\Comparative-factor-extraction-car\CRF++\outputT4.txt', 'r')
	f2 = open('F:\Comparative-factor-extraction-car\CRF++\\test_answer.txt', 'r')
	f3 = open('F:\Comparative-factor-extraction-car\CRF++\evaluateT4.txt', 'w')
	'''
	# 测试结果和答案是一样长的文件
	num = 0
	list2 = []
	...
	for line1 in f1.readlines():
		if line1 == '\n':
			...
			result = Defi_class.Evaluation('', result_1[0],result_1[1], '', result_1[2], result_1[3],  result_1[4],'')
			# print(result.word)
			list.append(result)
			i += 1
	# print(len(list))
	...
	for line2 in f2.readlines():
		...
		if line2 == '\n':
			...
			f3.write(
				list[num].index + ' ' + list[num].word + ' ' + list[num].pos+ ' ' + list[num].keyword+ ' ' + list[num].position + ' ' + list[num].result1 + ' ' + list[
					num].result2)
			f3.write('\n')
	return list

f4=open('F:\Comparative-factor-extraction-car\CRF++\\result2T4.txt', 'w')
f5=open('F:\Comparative-factor-extraction-car\CRF++\\result1T4.txt', 'w')
list=extraction()
