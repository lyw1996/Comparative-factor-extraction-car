...
import Defi_class
def extraction():
	f1 = open('F:\Comparative-factor-extraction-car\CRF++\outputT1.txt', 'r')
	f2 = open('F:\Comparative-factor-extraction-car\CRF++\\test_answer.txt', 'r')
	f3 = open('F:\Comparative-factor-extraction-car\CRF++\evaluateT1.txt', 'w')
	'''
	#测试结果和答案是一样长的文件
	num=0
	list2=[]
	...
	for line2 in f2.readlines():
		...
		if line2 == '\n':
			...
			f3.write(
				list[num].index + ' ' + list[num].word + ' ' + list[num].pos + ' ' + list[num].result1 + ' ' + list[
					num].result2)
			f3.write('\n')
	#print(len(list))
	list=np.array(list)
...
f4=open('F:\Comparative-factor-extraction-car\CRF++\\result2T1.txt', 'w')
f5=open('F:\Comparative-factor-extraction-car\CRF++\\result1T1.txt', 'w')
list=extraction()
#print(len(list))
list3=[]
for i2 in range(len(list)):
	list3.append(list[i2].index)
#print(list2)
list_index=sorted(set(list3))#句子去重，存放不重复的编号
#print(list_index)
target='SENTI'
